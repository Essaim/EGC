...
class DynamicGraphGRU(nn.Module):
    def __init__(self, input_size: int, dg_hidden_size: int, sample_num: int, conv_method: str):
        ...
        # self.mask_lienar = nn.Parameter(torch.randn(int(dg_hidden_size/2), int(dg_hidden_size/2)), requires_grad=True)
        if conv_method == 'softmax':
            self.conv = soft_conv(dg_hidden_size, sample_num, [3,0,1,2])
        elif conv_method == 'uniform':
            # self.conv = uni_conv(int(dg_hidden_size/2))
...
class DCRNNEncoder(nn.Module):
    def __init__(self, input_size: int, hidden_size: int, hidden_size_st:int,  hidden_size_dy: int,num_node: int, n_supports: int,
                 k_hop: int, n_layers: int, input_size_dy: int):
        ...
        self.input_size_dy = input_size_dy

    def forward(self, inputs: Tensor, supports: Tensor, node_fea_st:Tensor, dygraph_generator: nn.Module) -> Tensor:
        """
        :param inputs: tensor, [B, T, N, input_size]
        :param supports:  [B, N, N]
        ...
        for i_layer, cell in enumerate(self.encoder):
            for i_t in range(t):
                dy_graph, states_dy[i_layer] = dygraph_generator(inputs[i_t][..., :self.input_size_dy], states_dy[i_layer])
                dy_graph_list.append(dy_graph)
                inputs[i_t], states[i_layer] = cell(inputs[i_t], [supports, dy_graph], states[i_layer])
        return torch.stack(states), torch.stack(states_dy), torch.stack(dy_graph_list)


class DCRNNDecoder(nn.ModuleList):
    def __init__(self,  output_size: int, hidden_size: int, hidden_size_dy: int,
                 num_node: int,
                 n_supports: int, k_hop: int, n_layers: int, n_preds: int, input_size_dy: int):
    ...
    def forward(self, supports: Tensor, states: Tensor, first_states:Tensor, states_dy: Tensor,
                targets: Tensor = None, dygraph_generator: nn.Module = None, teacher_force: bool = 0.5) -> Tensor:
        """
        :param supports:  [B, N, N]
        ...
        for i_t in range(self.n_preds):
            for i_layer in range(n_layers):
                dy_graph, states_dy[i_layer] = dygraph_generator(inputs[..., :self.input_size_dy], states_dy[i_layer])
                dy_graph_list.append(dy_graph)
                inputs, states[i_layer] = self[i_layer](inputs, [supports, dy_graph], states[i_layer])
            inputs = self.out(inputs)
            new_outputs.append(inputs)
            if targets is not None and random.random() < teacher_force:
                inputs = targets[:, i_t]
        del dy_graph_list[0]
        return torch.stack(new_outputs, 1), torch.stack(dy_graph_list)
...
class regular_cal(nn.Module):
    def __init__(self, hidden_size = None, max_step= None, output_size= None, input_dim= None, n_supports= None):
        super(regular_cal, self).__init__()
        self.conv = Batch_GraphConv(input_dim=input_dim , output_dim =hidden_size, num_nodes = 111, n_supports = n_supports, max_step =max_step)
        self.out = nn.Linear(hidden_size, output_size)
    ...
    def forward(self, graph_dy, graph_st=None, inputs=None):
        ...
        conv_out = list()
        for g, i in zip(graph_dy, inputs[:-1,...]):
            states = self.conv(i, [graph_st, g])
            conv_out.append(self.out(states))  # (b, n, 1)
        conv_out = torch.stack(conv_out).reshape(t, b, n, -1)  # (t-1,b, n, c, dy/st)

        return conv_out, inputs[1:, ...]

class regular_cal_l2(nn.Module):
    def __init__(self):
        super(regular_cal_l2, self).__init__()

    def forward(self,graph_dy, graph_st=None, inputs=None):
        return graph_dy[:-1],graph_dy[1:]

class EGC_Stage2(nn.Module):
    def __init__(self,
                 n_pred: int,
                 hidden_size: int,
                 hidden_size_dy: int,
                 ...
                 node_fea: Tensor,
                 cl_decay_steps: int):
        super(EGC_Stage2, self).__init__()
        self.cl_decay_steps = cl_decay_steps
        self.node_encode = nf_encode(hidden_size_st, sample_num_st, st_conv_method, fc_dim)
        self.dygraph_generator = DynamicGraphGRU(inputs_size_dy, hidden_size_dy, sample_num_dy, dy_conv_method)
        self.encoder = DCRNNEncoder(input_dim, hidden_size, hidden_size_st,hidden_size_dy, num_nodes,
                                    n_supports, k_hop,
                                    n_rnn_layers, inputs_size_dy)
        self.decoder = DCRNNDecoder(output_dim, hidden_size, hidden_size_dy, num_nodes,
                                    n_supports, k_hop,
                                    n_rnn_layers, n_pred, inputs_size_dy)

        self.node_fea = node_fea
        self.regular_cal = regular_cal(hidden_size_simple, k_hop, output_dim, input_dim,n_supports)
        # self.regular_cal = regular_cal_l2()
    ...
    def forward(self, inputs: Tensor, i_targets: Tensor = None, targets: Tensor = None,
        ...
        states, states_dy, dygraph_en = self.encoder(inputs, support,x, self.dygraph_generator)
        outputs, dygraph_de = self.decoder(support, states, inputs[:,-1,:,:self.output_dim],states_dy, i_targets, self.dygraph_generator,
                                           self._compute_sampling_threshold(batch_seen))
        regular_term, regular_label = self.regular_cal(graph_dy = torch.cat([dygraph_en, dygraph_de], dim=0),
                                                       graph_st = support,
                                                        inputs = torch.cat([inputs,targets], dim= 1).transpose(0,1)
                                            )
